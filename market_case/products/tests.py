...
class ProductsTest(APITestCase):
    def setUp(self) -> NoReturn:
        ...
        self.sub_category_2 = Subcategory.objects.create(
            subcategory_name='Переноски',
            parent=self.category_2)
        self.product_2 = Product.objects.create(
            product_name='Перенсока красная',
            price=300,
            discount_price=280,
            product_balance=7,
            product_characteristics='Обычная переноска',
            product_category=self.category_2,
            product_subcategory=self.sub_category_2)

    def test_get_list(self) -> NoReturn:
        url = reverse('products:products')
        res = self.client.get(url)
        response_data = res.data.get('results')

        self.assertEqual(status.HTTP_200_OK, res.status_code)
        self.assertEqual(ProductSerializer(
            [self.product_1, self.product_2],
            many=True).data, response_data)

    def test_get_min_max_sum_price(self) -> NoReturn:
